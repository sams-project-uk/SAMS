...
namespace LARE {
    ...
    void remap(LARE3D& lare, simulationData& data, remapData& remap_data) {
        ...
        energy_flux<axis, &simulationData::energy_ion>(data, remap_data);
        ...
        pw::applyKernel(
            LAMBDA (idx_t ix, idx_t iy, idx_t iz) {
                ...
                RIndexer energy_ion { data.energy_ion };
                RIndexer rho { data.rho };
                RIndexer rho1 { remap_data.rho1 };
                RIndexer cv1 { data.cv1 };
                RIndexer flux { remap_data.flux };
                RIndexer cv2 { remap_data.cv2 };
                energy_ion(ia, ib, ic) = (
                    energy_ion(ia, ib, ic) * cv1(ia, ib, ic) * rho1(ia, ib, ic)
                    + flux(iam, ib, ic) - flux(ia, ib, ic)
                ) / (
                    cv2(ia, ib, ic) * rho(ia, ib, ic)
                );
    ...
    template void energy_flux<AxisName::X, &simulationData::energy_ion>(simulationData& data, remapData& remap_data);
    template void energy_flux<AxisName::Y, &simulationData::energy_electron>(simulationData& data, remapData& remap_data);
    template void energy_flux<AxisName::Y, &simulationData::energy_ion>(simulationData& data, remapData& remap_data);
    template void energy_flux<AxisName::Z, &simulationData::energy_electron>(simulationData& data, remapData& remap_data);
    template void energy_flux<AxisName::Z, &simulationData::energy_ion>(simulationData& data, remapData& remap_data);

    template void remap<AxisName::X>(LARE3D& lare, simulationData& data, remapData& remap_data);
    template void remap<AxisName::Y>(LARE3D& lare, simulationData& data, remapData& remap_data);
    template void remap<AxisName::Z>(LARE3D& lare, simulationData& data, remapData& remap_data);
}