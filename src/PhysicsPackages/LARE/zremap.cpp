...
namespace LARE
{
    ...
    void LARE3D<T_EOS>::remap_z(simulationData &data, remapData &remap_data)
    {
        ...
        vz_by_flux(data, remap_data); // Flux of by due to vz
        ...
        z_mass_flux(data, remap_data); // Mass flux in z-direction
        dm_z_bcs();    // Apply boundary conditions to mass flux
        ...
        z_mom_flux<&simulationData::vx>(data, remap_data); // Momentum flux in x-direction

        pw::applyKernel(
            LAMBDA(T_indexType ix, T_indexType iy, T_indexType iz) {
                T_indexType izm = iz - 1;
                data.vx(ix, iy, iz) = (remap_data.rho_v(ix, iy, iz) * data.vx(ix, iy, iz) * remap_data.cvc1(ix, iy, iz) + remap_data.flux(ix, iy, izm) - remap_data.flux(ix, iy, iz)) / (remap_data.cv2(ix, iy, iz) * remap_data.rho_v1(ix, iy, iz));
            },
            Range(0, data.nx), Range(0, data.ny), Range(0, data.nz));

        pw::fence();

        z_mom_flux<&simulationData::vy>(data, remap_data); // Momentum flux in y-direction
        pw::applyKernel(
            LAMBDA(T_indexType ix, T_indexType iy, T_indexType iz) {
                T_indexType izm = iz - 1;
                data.vy(ix, iy, iz) = (remap_data.rho_v(ix, iy, iz) * data.vy(ix, iy, iz) * remap_data.cvc1(ix, iy, iz) + remap_data.flux(ix, iy, izm) - remap_data.flux(ix, iy, iz)) / (remap_data.cv2(ix, iy, iz) * remap_data.rho_v1(ix, iy, iz));
            },
            Range(0, data.nx), Range(0, data.ny), Range(0, data.nz));
        pw::fence();

        z_mom_flux<&simulationData::vz>(data, remap_data);  // Momentum flux in z-direction
        pw::applyKernel(
            LAMBDA(T_indexType ix, T_indexType iy, T_indexType iz) {
                T_indexType izm = iz -1;
                data.vz(ix, iy, iz) = (remap_data.rho_v(ix, iy, iz) * data.vz(ix, iy, iz) * remap_data.cvc1(ix, iy, iz) + remap_data.flux(ix, iy, izm) - remap_data.flux(ix, iy, iz)) / (remap_data.cv2(ix, iy, iz) * remap_data.rho_v1(ix, iy, iz));
            },
            Range(0, data.nx), Range(0, data.ny), Range(0, data.nz));
        pw::fence();
    ...
    template<typename T_EOS>
    void LARE3D<T_EOS>::vz_bx_flux(simulationData &data, remapData &remap_data)
    {
        using Range = pw::Range;
        pw::applyKernel(
            LAMBDA(T_indexType ix, T_indexType iy, T_indexType iz) {
    ...
    // Evans & Hawley constrained transport remap of vz*Bx fluxes
    template<typename T_EOS>
    void LARE3D<T_EOS>::vz_by_flux(simulationData &data, remapData &remap_data)
    {
        using Range = pw::Range;
        pw::applyKernel(
        LAMBDA(T_indexType ix, T_indexType iy, T_indexType iz) {
    ...
    template<typename T_EOS>
    void LARE3D<T_EOS>::z_mass_flux(simulationData &data, remapData &remap_data)
    {
        using Range = pw::Range;
        pw::applyKernel(
            LAMBDA(T_indexType ix, T_indexType iy, T_indexType iz) {
    ...
    template <typename T_EOS>
    template <auto mPtr>
    void LARE3D<T_EOS>::z_energy_flux(simulationData &data, remapData &remap_data)
    {
        using Range = pw::Range;
        pw::applyKernel(
            LAMBDA(T_indexType ix, T_indexType iy, T_indexType iz) {
    ...
    template <typename T_EOS>
    template <auto mPtr>
    void LARE3D<T_EOS>::z_mom_flux(simulationData &data, remapData &remap_data)
    {
        using Range = pw::Range;
