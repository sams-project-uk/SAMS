...
namespace LARE
{
    ...
    void LARE3DST<T_EOS>::remap_x(simulationData &data, remapData &remap_data)
    {
        ...
        dm_x_bcs();    // Apply boundary conditions to mass flux
    ...
    template<typename T_EOS>
    void LARE3DST<T_EOS>::vx_by_flux(simulationData &data, remapData &remap_data)
    {
        using Range = pw::Range;
    ...
    template<typename T_EOS>
    void LARE3DST<T_EOS>::vx_bz_flux(simulationData &data, remapData &remap_data)
    {
        using Range = pw::Range;
        pw::applyKernel(LAMBDA(T_indexType ix, T_indexType iy, T_indexType iz) {
        T_indexType ixm = ix - 1;
    ...
    template<typename T_EOS>
    void LARE3DST<T_EOS>::x_mass_flux(simulationData &data, remapData &remap_data)
    {
        using Range = pw::Range;
        pw::applyKernel(
            LAMBDA(T_indexType ix, T_indexType iy, T_indexType iz) {
    ...
    template<typename T_EOS>
    template <auto mPtr>
    void LARE3DST<T_EOS>::x_energy_flux(simulationData &data, remapData &remap_data)
    {
        using Range = pw::Range;
        pw::applyKernel(
            LAMBDA(T_indexType ix, T_indexType iy, T_indexType iz) {
                ...
                T_dataType dmu = std::abs(data.dm(ix, iy, iz)) / dxbu / rhou;
    ...
    template<typename T_EOS>
    template <auto mPtr>
    void LARE3DST<T_EOS>::x_mom_flux(simulationData &data, remapData &remap_data)
    {
        using Range = pw::Range;
        pw::applyKernel(
            LAMBDA(T_indexType ix, T_indexType iy, T_indexType iz) {
