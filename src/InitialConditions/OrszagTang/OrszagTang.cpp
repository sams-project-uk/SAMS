...
namespace examples
{
    ...
    void OrszagTang<T_EOS>::attachBoundaryConditions(const std::string &varName, SAMS::harness &harness)
    {
     ...
     */
    template<typename T_EOS>
    void OrszagTang<T_EOS>::controlVariables(typename LARE::LARE3D<T_EOS>::simulationData &data)
    {
        ...
        data.y_max = 2.0 * M_PI;
        data.z_min = 0.0;
        data.z_max = 2.0 * M_PI * double(data.nz) / double(data.nx); // Keep cells roughly cubic
     ...
     */
    template<typename T_EOS>
    void OrszagTang<T_EOS>::setDomain(SAMS::harness &harness, typename LARE::LARE3D<T_EOS>::simulationData &data)
    {
     ...
     */
    template<typename T_EOS>
    void OrszagTang<T_EOS>::setBoundaryConditions(SAMS::harness &harness)
    {
     ...
     */
    template<typename T_EOS>
    void OrszagTang<T_EOS>::initialConditions(SAMS::harness &harnessRef, typename LARE::LARE3D<T_EOS>::simulationData &data)
    {
     ...
     */
    template<typename T_EOS>
    void OrszagTang<T_EOS>::queryTerminate(bool &terminate, typename LARE::LARE3D<T_EOS>::simulationData &data, SAMS::timeState &tData)
    {
     ...
     */
    template<typename T_EOS>
    void OrszagTang<T_EOS>::queryOutput(bool &shouldOutput, typename LARE::LARE3D<T_EOS>::simulationData &data, SAMS::timeState &tData)
    {
