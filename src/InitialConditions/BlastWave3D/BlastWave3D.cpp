...
namespace examples
{
    ...
    void BlastWave3D<T_EOS>::attachBoundaryConditions(const std::string &varName, SAMS::harness &harness)
    {
     ...
     */
    template<typename T_EOS>
    void BlastWave3D<T_EOS>::controlVariables(typename LARE::LARE3D<T_EOS>::simulationData &data)
    {

        data.t_end = 1.5; // End time of the simulation
     ...
     */
    template<typename T_EOS>
    void BlastWave3D<T_EOS>::setDomain(SAMS::harness &harness, typename LARE::LARE3D<T_EOS>::simulationData &data)
    {
     ...
     */
    template<typename T_EOS>
    void BlastWave3D<T_EOS>::setBoundaryConditions(SAMS::harness &harness)
    {
     ...
     */
    template<typename T_EOS>
    void BlastWave3D<T_EOS>::initialConditions(SAMS::harness &harnessRef, typename LARE::LARE3D<T_EOS>::simulationData &data)
    {
        ...
        pw::applyKernel(
            LAMBDA(SAMS::T_indexType ix, SAMS::T_indexType iy, SAMS::T_indexType iz) {
                ...
                T_dataType radius= std::sqrt(xc(ix)*xc(ix)+yc(iy)*yc(iy)+zc(iz)*zc(iz));
     ...
     */
    template<typename T_EOS>
    void BlastWave3D<T_EOS>::queryTerminate(bool &terminate, typename LARE::LARE3D<T_EOS>::simulationData &data, SAMS::timeState &tData)
    {
     ...
     */
    template<typename T_EOS>
    void BlastWave3D<T_EOS>::queryOutput(bool &shouldOutput, typename LARE::LARE3D<T_EOS>::simulationData &data, SAMS::timeState &tData)
    {
