...
namespace portableWrapper
{
         ...
         */
        template <executionSpace space = executionSpace::accelerated, typename T_func, typename... T_ranges>
        ...
        {
                applyKernel<executionSpace::host>(std::forward<T_func>(func), ranges...);
        }
         ...
         * */
        template <executionSpace space = executionSpace::accelerated, typename T_ret = void, typename T_data_in, typename T_mapper, typename T_reducer, typename... T_ranges>
        inline auto applyReduction(T_mapper mapper, T_reducer reducer, T_data_in initialValue, T_ranges... ranges)
        {

                using T_inferred = std::decay_t<std::tuple_element_t<0, typename far::callableTraits<T_reducer>::params>>;
                using T_data = std::conditional_t<std::is_void_v<T_ret>, T_inferred, T_ret>;
         ...
         * @param ranges Ranges over which to apply the reduction. The number of ranges must match the rank of the data.
         * */
        template <typename T_ret = void, typename T_data_in, typename T_mapper, typename T_reducer, typename... T_ranges>
        ...
        {
                return applyReduction<executionSpace::host, T_ret>(std::forward<T_mapper>(mapper), std::forward<T_reducer>(reducer), initialValue, ranges...);
        }

        /**
         * Function to provide a fence point for the parallel operations.
         */
        template <executionSpace space = executionSpace::accelerated>
        inline void fence()
        {
                if constexpr (space == executionSpace::host)
                {
...
#endif
                // Always initialize OpenMP
                openmp::initialize(argc, argv);
        }

        /**
         * Function to print information about the parallelization mode.
         */

        inline void printParallelizationInfo()
        {
